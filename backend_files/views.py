...
from core.kwikpik import kwikpik
...
from django.contrib.auth.decorators import login_required
from core.decorators import vendor_required, user_required


def media_thumb(request):
    """
    Generate/cache a lightweight thumbnail from a media file.
    Query params:
      - src: media path or absolute URL to this same host's /media/... file
      - w: target width (default 420)
      - h: optional target height (maintains aspect if omitted)
      - q: quality (default 72)
      - fmt: webp|jpeg (default webp)
    """
    src = request.GET.get("src", "") or request.GET.get("url", "") or request.GET.get("path", "")
    if not src:
        return HttpResponseBadRequest("Missing src")

    try:
        width = max(48, min(1600, int(request.GET.get("w", "420"))))
    except Exception:
        width = 420
    try:
        height_raw = request.GET.get("h")
        height = max(48, min(1600, int(height_raw))) if height_raw else None
    except Exception:
        height = None
    try:
        quality = max(35, min(95, int(request.GET.get("q", "72"))))
    except Exception:
        quality = 72

    fmt = str(request.GET.get("fmt", "webp")).strip().lower()
    ...
    if not media_root:
        return HttpResponseBadRequest("Media not configured")

    parsed = urlparse(src)
    candidate_path = parsed.path if parsed.scheme else src
    candidate_path = unquote(candidate_path or "")

    if candidate_path.startswith(media_url):
        rel_path = candidate_path[len(media_url):]
    elif media_url.startswith("/") and candidate_path.startswith(f"/ng{media_url}"):
        # Support deployments where app routes are prefixed with /ng.
        rel_path = candidate_path[len(f"/ng{media_url}"):]
    elif "/media/" in candidate_path:
        # Fallback for mixed absolute/relative media path shapes.
        rel_path = candidate_path.split("/media/", 1)[1]
    else:
        rel_path = candidate_path.lstrip("/")

    rel_path = os.path.normpath(rel_path).replace("\\", "/")
    if rel_path.startswith("../") or rel_path.startswith("..\\"):
        return HttpResponseBadRequest("Invalid media path")

    source_file = os.path.abspath(os.path.join(media_root, rel_path))
    if not source_file.startswith(media_root) or not os.path.exists(source_file):
        return HttpResponseBadRequest("Media not found")

    cache_key = hashlib.sha1(f"{rel_path}|{width}|{height}|{quality}|{fmt}".encode("utf-8")).hexdigest()
    thumb_dir = os.path.join(media_root, "_thumb_cache")
    os.makedirs(thumb_dir, exist_ok=True)
    ext = "webp" if fmt == "webp" else "jpg"
    thumb_file = os.path.join(thumb_dir, f"{cache_key}.{ext}")

    if not os.path.exists(thumb_file):
        try:
            from PIL import Image, ImageOps  # Lazy import to avoid hard crash if missing
        except Exception:
            # Fallback: return original file when pillow is not available
            original = open(source_file, "rb")
            response = FileResponse(original)
        ...
        with Image.open(source_file) as img:
            ...
            if height:
                ...
                resized = img.resize((width, target_h), Image.Resampling.LANCZOS)

            save_kwargs = {"quality": quality, "optimize": True}
            if fmt == "webp":
                save_kwargs["method"] = 6
                resized.save(thumb_file, "WEBP", **save_kwargs)
            else:
                if resized.mode == "RGBA":
                    resized = resized.convert("RGB")
                resized.save(thumb_file, "JPEG", progressive=True, **save_kwargs)

    thumb_stream = open(thumb_file, "rb")
    content_type = "image/webp" if fmt == "webp" else "image/jpeg"
    response = FileResponse(thumb_stream, content_type=content_type)
    response["Cache-Control"] = "public, max-age=31536000, immutable"
    return response


def media_thumb_key(request, key: str):
    """
    Path-safe wrapper around media_thumb.
    Accepts base64url-encoded src in route to avoid query-string stripping by redirects/proxies.
    """
    decoded_src = ""
    try:
        # Backward-compatible: base64url key support.
        pad_len = (-len(key)) % 4
        padded = f"{key}{'=' * pad_len}"
        decoded_src = base64.urlsafe_b64decode(padded.encode("utf-8")).decode("utf-8")
    except Exception:
        try:
            # Preferred path-safe key format from mobile app.
            decoded_src = unquote(key.replace("~", "%"))
        except Exception:
            return HttpResponseBadRequest("Invalid key")

    mutable_query = request.GET.copy()
    mutable_query["src"] = decoded_src
    request.GET = mutable_query
    return media_thumb(request)
    
def thetest(request):
    return render(request, "core/test.html")
    
def update_flash_time(request):
...
#     spent = Cart.objects.filter(user=request.user).aggregate(Sum('product__price'))['product__price__sum']
#     check_qty = Cart.objects.filter(user=request.user).aggregate(Sum('qty'))['qty__sum']
#     if request.session.get('new_price_after_coupon'):
#         thecod = request.session.get('thecoupon')
#         thecode = Coupon.objects.get(coupon_code=thecod)
#         totl = request.session.get('new_price_after_coupon')
#         test = float(totl)
#     else:
#         test = Cart.objects.filter(user=request.user).aggregate(total=Sum(F('product__price') * F('qty')))['total'] or 0
...
#     cart_total = float(d) * float(q)
    
#     print("This is the total:", spent)
#     print("This is the qty:", check_qty)
...
#     # date_only = datetime.fromisoformat(timestamp.rstrip("Z")).date()
...
from trenva.push_notifications import send_push_to_user


logger = logging.getLogger(__name__)

def _create_user_notification(user, notification_type, title, message, order=None, order_item=None):
    try:
        ...
        send_push_to_user(user, title=title, body=message, data={"type": "important"})
        print("message sent :", title, message)
    except Exception as e:
        logger.error(f"Failed creating notification for user {getattr(user, 'id', None)}: {str(e)}")
...
def checkout(request, order_id=None):
    ...
    # Get delivery method from session (default to Door Step Delivery)
        ...
        if request.method == "POST":
            ...
            if not address_id:
                ...
                context = {
                    ...
                    "delivery_fee": 0,
                    "kwikpik_price_lagos": settings.KWIKPIK_PRICE_LAGOS,
                    "kwikpik_price_other": settings.KWIKPIK_PRICE_OTHER,
                }
            ...
            try:
                ...
                delivery_fee = kwikpik.calculate_delivery_fee(selected_address.state)
                request.session['kwikpik_delivery_fee'] = float(delivery_fee)
...
#     delivery_fee = 0
#     kwikpik_delivery_fee = 0
#     trippa_delivery_fee = 0
#     selected_state = None
...
#             # Get updated delivery method from POST
#             delivery_method = request.POST.get("delivery_method", "Door Step Delivery")
...
#                 # Calculate delivery fees based on selected method
#                 delivery_fee = 0
                
#                 if delivery_method == "Door Step Delivery":
#                     # Kwikpik fee
#                     kwikpik_delivery_fee = kwikpik.calculate_delivery_fee(selected_address.state)
#                     request.session['kwikpik_delivery_fee'] = float(kwikpik_delivery_fee)
#                     delivery_fee = kwikpik_delivery_fee
                    
#                 elif delivery_method == "trippa_delivery":
#                     # Trippa fee calculation
#                     try:
#                         trippa = TrippaDelivery()
                        
#                         # Calculate total weight and value from cart
#                         total_weight = 0
#                         total_value = 0
#                         if order_id and order_items:
#                             for item in order_items:
#                                 total_weight += 0.5 * item.qty
#                                 total_value += float(item.price) * item.qty
#                         else:
#                             for c in cart:
#                                 total_weight += 0.5 * c.qty
#                                 total_value += float(c.product.price) * c.qty
                        
#                         quote = trippa.get_quote(
#                             pickup_address=settings.TRENVA_PICKUP_ADDRESS,
#                             delivery_address=f"{selected_address.address}, {selected_address.city}, {selected_address.state}",
#                             item_description="Items",
#                             weight=total_weight,
#                             value=total_value
#                         )
#                         if quote.get("success"):
#                             trippa_delivery_fee = quote.get("delivery_cost", 0)
#                             request.session['trippa_delivery_fee'] = float(trippa_delivery_fee)
#                             delivery_fee = trippa_delivery_fee
#                     except Exception as e:
#                         logger.error(f"Trippa quote error: {str(e)}")
...
def get_cities_json(request):
    ...
    cities = list(
        DeliveryZone.objects.filter(
            state_name__iexact=state.strip(),
...
def get_delivery_fee_json(request):
    """API endpoint to get delivery fee for a specific state/city."""
    state = request.GET.get("state")
    ...
    if normalized_state == "lagos":
        delivery_fee = Decimal(str(getattr(settings, "LAGOS_DELIVERY_FEE", "3900.00")))
    else:
        delivery_fee = Decimal(str(getattr(settings, "OTHER_STATE_DELIVERY_FEE", "9000.00")))
...
def place_order(request):
    ...
    kwikpik_delivery_fee = request.session.get('kwikpik_delivery_fee', 0)
    ...
    try:
        ...
        total_with_delivery = total + Decimal(str(kwikpik_delivery_fee))
        ...
        if request.session.get('new_price_after_coupon'):
            try:
                ...
                try:
                    ...
                    coupon_used = True
                    total_with_delivery = new_price + Decimal(str(kwikpik_delivery_fee))
                except Coupon.DoesNotExist:
        ...
        # For Kwikpik - track items per vendor
        vendor_items_map = {}
        vendor_weight_map = {}
        ...
        for p_id, item in cart_data.items():
            ...
            try:
                ...
                # Track per vendor (no weight calculation needed)
                ...
                vendor_items_map[vendor_id].append({
                    ...
                    'size_category': product.size_category if product and hasattr(product, 'size_category') else 'small'
                })
                ...
                # Track per vendor for non-product items
                if vendor_id not in vendor_items_map:
                    vendor_items_map[vendor_id] = []
                vendor_items_map[vendor_id].append({
                    'name': item['title'][:50],
                    'quantity': qty,
                    'total': float(price) * qty,
                    'size_category': 'small'  # Default for unknown
                })
            ...
            # Color-Size combination stock reduction
        ...
        # ========== KWIKPIK MULTI-VENDOR BULK ORDER ==========
        if delivery_method == "Door Step Delivery" and kwikpik_delivery_fee > 0 and thepayment:
            try:
                from django.conf import settings
                import requests
                
                # Get unique vendors from cart_data
                unique_vendors = set()
                for item in all_order_items:
                    if item.vendor_id:
                        unique_vendors.add(item.vendor_id)
                
                # Check if any product needs a VAN (large size category)
                needs_van = any(
                    item_data.get('size_category') == 'large' 
                    for vendor_data in vendor_items_map.values() 
                    for item_data in vendor_data
                )
                
                # If only ONE vendor, use simple unified endpoint
                if len(unique_vendors) == 1:
                    # Get vendor pickup info
                    vendor_pickup_address = getattr(settings, 'KWIKPIK_PICKUP_ADDRESS', 'Trenva Warehouse, Lagos')
                    vendor_pickup_contact = getattr(settings, 'KWIKPIK_PICKUP_CONTACT', 'Warehouse Team')
                    vendor_pickup_phone = getattr(settings, 'KWIKPIK_PICKUP_PHONE', '+2340000000000')
                    kwikpik_items = []
                    
                    try:
                        vendor = Vendor.objects.get(vid=list(unique_vendors)[0])
                        if vendor.physical_address:
                            vendor_pickup_address = vendor.physical_address
                        if vendor.business_name:
                            vendor_pickup_contact = vendor.business_name
                        if vendor.phone_number:
                            vendor_pickup_phone = vendor.phone_number
                    except Vendor.DoesNotExist:
                        pass
                    
                    # Build items list
                    for item in all_order_items:
                        kwikpik_items.append({
                            'name': item.item[:50],
                            'quantity': item.qty
                        })
                    
                    # Prepare order data for unified endpoint
                    unified_payload = {
                        "orders": [{
                            "merchantPackageNumber": f"TRENVA-{order.id}",
                            "pickupAddress": vendor_pickup_address,
                            "pickupContactName": vendor_pickup_contact,
                            "pickupContactPhone": vendor_pickup_phone,
                            "deliveryAddress": f"{filladdress.address}, {filladdress.city}, {filladdress.state}",
                            "deliveryContactName": f"{filladdress.first_name} {filladdress.last_name}",
                            "deliveryContactPhone": filladdress.phone,
                            "deliveryContactEmail": email,
                            "description": f"Order #{order.id}",
                            "quantity": len(all_order_items),
                            "packageValue": float(total),
                            "items": kwikpik_items
                        }]
                    }
                    
                    # Add vehicleType ONLY if large items are present
                    if needs_van:
                        unified_payload["orders"][0]["vehicleType"] = "VAN"
                    
                    headers = {
                        'x-api-key': getattr(settings, 'KWIKPIK_API_KEY', ''),
                        'Content-Type': 'application/json'
                    }
                    url = f"{getattr(settings, 'KWIKPIK_BASE_URL', 'https://logistics-api.kwikpik.io')}/api/v2/orders/unified"
                    
                    response = requests.post(url, headers=headers, json=unified_payload, timeout=30)
                    
                    if response.status_code == 200 or response.status_code == 201:
                        data = response.json()
                        successful = data.get('data', {}).get('successful', [])
                        if successful:
                            order_info = successful[0]
                            package_info = order_info.get('packageInformation', {})
                            tracking_id = package_info.get('trackingId')
                            if tracking_id:
                                order.kwikpik_tracking_id = tracking_id
                                order.kwikpik_order_code = order_info.get('orderCode')
                                order.save()
                                logger.info(f"Kwikpik order created for order {order.id}: {tracking_id}")
                        else:
                            failed = data.get('data', {}).get('failed', [])
                            if failed:
                                logger.error(f"Kwikpik failed: {failed[0].get('error')}")
                    else:
                        logger.error(f"Kwikpik API error: {response.status_code}")
                
                # If MULTIPLE vendors, use bulk endpoint with PACKAGES array
                else:
                    bulk_orders = []
                    tracking_data = {}
                    
                    for vendor_id, items_data in vendor_items_map.items():
                        # Get vendor pickup info
                        vendor_pickup_address = getattr(settings, 'KWIKPIK_PICKUP_ADDRESS', 'Trenva Warehouse, Lagos')
                        vendor_pickup_contact = getattr(settings, 'KWIKPIK_PICKUP_CONTACT', 'Warehouse Team')
                        vendor_pickup_phone = getattr(settings, 'KWIKPIK_PICKUP_PHONE', '+2340000000000')
                        ...
                        try:
                            ...
                            if vendor.business_name:
                                ...
                                vendor_pickup_contact = vendor.name
                        ...
                        vendor_items_list = vendor_items_map.get(vendor_id, [])
                        vendor_total_value = sum(item.get('total', 0) for item in vendor_items_list)
                        
                        # Check if this vendor has any large items
                        vendor_needs_van = any(item.get('size_category') == 'large' for item in vendor_items_list)
                        
                        # Build order object
                        order_obj = {
                            "orderRef": f"TRENVA-{order.id}-V{vendor_id}",
                            "pickupAddress": vendor_pickup_address,
                            "pickupContactName": vendor_pickup_contact,
                            "pickupContactPhone": vendor_pickup_phone,
                            "packages": [{
                                "merchantPackageNumber": f"PKG-{vendor_id}",
                                "deliveryAddress": f"{filladdress.address}, {filladdress.city}, {filladdress.state}",
                                "deliveryContactName": f"{filladdress.first_name} {filladdress.last_name}",
                                "deliveryContactPhone": filladdress.phone,
                                "deliveryContactEmail": email,
                                "description": f"Order #{order.id} - {vendor_name}",
                                "packageValue": vendor_total_value,
                                "items": [{'name': item['name'], 'quantity': item['quantity']} for item in vendor_items_list]
                            }]
                        }
                        
                        # Add vehicleType ONLY if this vendor has large items
                        if vendor_needs_van:
                            order_obj["vehicleType"] = "VAN"
                        
                        bulk_orders.append(order_obj)
                    
                    if bulk_orders:
                        bulk_payload = {"orders": bulk_orders}
                        headers = {
                            'x-api-key': getattr(settings, 'KWIKPIK_API_KEY', ''),
                            'Content-Type': 'application/json'
                        }
                        url = f"{getattr(settings, 'KWIKPIK_BASE_URL', 'https://logistics-api.kwikpik.io')}/api/v2/orders/bulk"
                        
                        response = requests.post(url, headers=headers, json=bulk_payload, timeout=30)
                        
                        if response.status_code == 200 or response.status_code == 201:
                            data = response.json()
                            
                            # Process successful orders - tracking ID is inside packages array
                            for successful in data.get('data', {}).get('successful', []):
                                order_ref = successful.get('orderRef')
                                packages = successful.get('packages', [])
                                if packages:
                                    tracking_id = packages[0].get('trackingId')
                                    order_code = successful.get('orderCode')
                                    
                                    # Extract vendor_id from order_ref
                                    if '-V' in order_ref:
                                        vendor_id_extracted = order_ref.split('-V')[-1]
                                        tracking_data[vendor_id_extracted] = {
                            ...
                            if tracking_data:
                                order.kwikpik_tracking_data = tracking_data
                                # Also save first tracking ID to old field for compatibility
                                first_tracking = list(tracking_data.values())[0].get('tracking_id') if tracking_data else None
                                order.kwikpik_tracking_id = first_tracking
                                order.save()
                                logger.info(f"Kwikpik bulk order created for order {order.id}: {len(tracking_data)} vendors")
                                
                            # Log failures
                            for failed in data.get('data', {}).get('failed', []):
                                logger.error(f"Kwikpik failed for {failed.get('merchantPackageNumber', 'unknown')}: {failed.get('error')}")
                        else:
                            logger.error(f"Kwikpik bulk API error: {response.status_code} - {response.text[:200]}")
                    
            except Exception as e:
                logger.error(f"Kwikpik error for order {order.id}: {str(e)}")
                import traceback
                traceback.print_exc()
        # ========== END KWIKPIK INTEGRATION ==========
        
        # Skip Trippa for now - commented out
        # if delivery_method == "trippa_delivery" and thepayment:
        #     ... Trippa code commented out ...
        ...
        try:
            _create_user_notification(
                ...
                message=f"Your order #{order.oid} has been placed and is now being processed.",
                order=order,
            )
        except Exception as e:
            logger.error(f"Failed to create order placed notification for user {user.id}: {str(e)}")
...
#     # Get delivery fees from session
#     kwikpik_delivery_fee = request.session.get('kwikpik_delivery_fee', 0)
#     trippa_delivery_fee = request.session.get('trippa_delivery_fee', 0)
...
#         # Add delivery fee to total based on selected method
#         if delivery_method == "Door Step Delivery":
#             total_with_delivery = total + Decimal(str(kwikpik_delivery_fee))
#         elif delivery_method == "trippa_delivery":
#             total_with_delivery = total + Decimal(str(trippa_delivery_fee))
#         else:
...
#                         total_with_delivery = new_price + Decimal(str(kwikpik_delivery_fee))
#                     else:
#                         total_with_delivery = new_price + Decimal(str(trippa_delivery_fee))
#                 except Coupon.DoesNotExist:
#                     logger.warning(f"Invalid coupon code {thecode} for user {user.id}")
#                     request.session.pop('new_price_after_coupon', None)
#                     request.session.pop('thecoupon', None)
...
#         # Create order items and build vendor items map
#         vendor_orders = {}
#         all_order_items = []
#         stock_reduction_failures = []
        
#         # For delivery services - track items per vendor
#         vendor_items_map = {}
...
#                 # Track per vendor for delivery services
#                 if vendor_id not in vendor_items_map:
#                     vendor_items_map[vendor_id] = {
#                         'vendor_name': vendor_name,
#                         'items': [],
...
#                 vendor_items_map[vendor_id]['items'].append({
#                     'name': item['title'][:50],
#                     'quantity': qty,
#                     'price': float(Decimal(str(item['price']).replace(',', ''))),
#                     'size_category': product.size_category if hasattr(product, 'size_category') else 'small'
#                 })
#                 vendor_items_map[vendor_id]['total_weight'] += 0.5 * qty
#                 vendor_items_map[vendor_id]['total_value'] += float(Decimal(str(item['price']).replace(',', ''))) * qty
...
#             # Color stock reduction
#             selected_color = item.get('product_color', '')
#             if selected_color and selected_color != "Default" and product:
#                 try:
#                     color = ProductColor.objects.filter(
#                         color_name=selected_color,
#                         product=product
...
#             # Size stock reduction
#             selected_size = item.get('product_size', '')
#             if selected_size and selected_size != "Default" and product:
#                 try:
#                     size_obj = ProductSize.objects.filter(
#                         size=selected_size,
#                         product=product
...
#         # ========== KWIKPIK DELIVERY (Door Step Delivery) ==========
#         if delivery_method == "Door Step Delivery" and kwikpik_delivery_fee > 0 and thepayment:
#             try:
#                 from django.conf import settings
#                 import requests
...
#                 # Get unique vendors from cart_data
#                 unique_vendors = set(vendor_items_map.keys())
                
#                 # Check if any product needs a VAN (large size category)
#                 needs_van = any(
#                     item.get('size_category') == 'large' 
#                     for vendor_data in vendor_items_map.values() 
...
#                 # If only ONE vendor, use simple unified endpoint
#                 if len(unique_vendors) == 1:
#                     vendor_id = list(unique_vendors)[0]
#                     vendor_data = vendor_items_map[vendor_id]
...
#                     # Build items list
#                     kwikpik_items = [{'name': item['name'], 'quantity': item['quantity']} for item in vendor_data['items']]
...
#                             "deliveryContactEmail": email,
#                             "description": f"Order #{order.id}",
#                             "quantity": len(vendor_data['items']),
#                             "packageValue": vendor_data['total_value'],
#                             "items": kwikpik_items
#                         }]
#                     }
...
#                 # If MULTIPLE vendors, use bulk endpoint
#                 else:
#                     bulk_orders = []
#                     tracking_data = {}
                    
#                     for vendor_id, vendor_data in vendor_items_map.items():
#                         vendor_pickup_address = getattr(settings, 'KWIKPIK_PICKUP_ADDRESS', 'Trenva Warehouse, Lagos')
#                         vendor_pickup_contact = getattr(settings, 'KWIKPIK_PICKUP_CONTACT', 'Warehouse Team')
#                         vendor_pickup_phone = getattr(settings, 'KWIKPIK_PICKUP_PHONE', '+2340000000000')
...
#                         vendor_needs_van = any(item.get('size_category') == 'large' for item in vendor_data['items'])
...
#                                 "description": f"Order #{order.id} - {vendor_name}",
#                                 "packageValue": vendor_data['total_value'],
#                                 "items": [{'name': item['name'], 'quantity': item['quantity']} for item in vendor_data['items']]
#                             }]
#                         }
...
#                                     tracking_id = packages[0].get('trackingId')
#                                     if tracking_id:
#                                         for vid in vendor_items_map:
#                                             if f"V{vid}" in order_ref:
#                                                 tracking_data[vid] = {
#                                                     'tracking_id': tracking_id,
#                                                     'order_code': successful.get('orderCode'),
#                                                     'status': 'pending'
...
#                                 order.kwikpik_tracking_data = tracking_data
#                                 order.save()
#                                 logger.info(f"Kwikpik bulk orders created for order {order.id}")
            
#             except Exception as e:
#                 logger.error(f"Kwikpik delivery creation failed: {str(e)}")
        
#         # ========== TRIPPA DELIVERY (Trippa Express Delivery) ==========
#         if delivery_method == "trippa_delivery" and thepayment:
#             try:
#                 from core.services.trippa import TrippaDelivery
...
#                 # Create delivery using vendor_items_map (same pattern as Kwikpik)
#                 result = trippa.create_delivery_from_vendor_map(
#                     vendor_items_map=vendor_items_map,
#                     customer_address=filladdress,
#                     customer_email=email,
#                     total_delivery_fee=trippa_delivery_fee
#                 )
                
#                 if result.get("success"):
#                     # Save tracking data
#                     order.trippa_tracking_data = result.get('tracking_data', {})
                    
#                     # Save single tracking ID for backward compatibility
#                     deliveries = result.get('deliveries', [])
#                     if deliveries:
#                         order.trippa_tracking_id = deliveries[0].get('tracking_number')
                    
#                     order.save()
#                     logger.info(f"Trippa delivery created for order {order.id}: {result}")
#                     messages.info(request, f"Tracking number(s) assigned for your Trippa delivery.")
#                 else:
#                     logger.error(f"Trippa delivery failed for order {order.id}: {result.get('error')}")
#                     messages.warning(request, "Trippa delivery creation pending. You'll receive tracking info shortly.")
                    
#             except Exception as e:
#                 logger.error(f"Trippa delivery creation failed for order {order.id}: {str(e)}", exc_info=True)
#                 messages.warning(request, "Trippa delivery creation pending. You'll receive tracking info shortly.")
...
#             'flutterwave_paid', 'paystack_paid', 'wallet_paid', 'order_id',
#             'coupon_id', 'coupon_code', 'discount_amount', 'cart_total', 'final_total',
#             'kwikpik_delivery_fee', 'trippa_delivery_fee',
#         ]
...
#     # ========== GET RETURN IMAGES ==========
#     from core.models import ReturnImage
#     return_images = ReturnImage.objects.filter(return_request=order)
#     # ========== END ==========
    
#     # ✅ ADDED: Get Kwikpik tracking info if available (single tracking)
#     kwikpik_tracking = None
#     if order.kwikpik_tracking_id:
#         try:
#             from core.kwikpik import kwikpik
#             kwikpik_tracking = kwikpik.track_order(order.kwikpik_tracking_id)
#         except Exception as e:
#             logger.error(f"Kwikpik tracking error for order {id}: {str(e)}")
#             kwikpik_tracking = {'success': False, 'error': str(e)}
    
#     # ✅ ADDED: Get multi-vendor tracking data
#     kwikpik_tracking_data = None
#     if order.kwikpik_tracking_data:
#         kwikpik_tracking_data = order.kwikpik_tracking_data
#         # Optionally fetch live status for each tracking ID
#         try:
#             from core.kwikpik import kwikpik
#             for vendor_id, tracking_info in kwikpik_tracking_data.items():
#                 if tracking_info.get('tracking_id'):
#                     live_status = kwikpik.track_order(tracking_info['tracking_id'])
#                     if live_status.get('success'):
#                         tracking_info['live_status'] = live_status.get('status')
#                         tracking_info['rider_name'] = live_status.get('rider_name')
#                         tracking_info['rider_phone'] = live_status.get('rider_phone')
#         except Exception as e:
#             logger.error(f"Error fetching live tracking status: {str(e)}")
...
#                             'id': vendor.vid,
#                             'physical_address': vendor.physical_address if hasattr(vendor, 'physical_address') else None,
#                             'phone_number': vendor.phone_number if hasattr(vendor, 'phone_number') else None,
#                         }
#                     except Vendor.DoesNotExist:
#                         vendor_info = {
...
#         "kwikpik_tracking": kwikpik_tracking,  # Single tracking (backward compatible)
#         "kwikpik_tracking_data": kwikpik_tracking_data,  # Multi-vendor tracking
#         "return_images": return_images,  # Return images
#         "has_return_images": return_images.exists(),  # Check if return has images
#     }
...
def order_details(request, id):
    ...
    print("=" * 50)
    print(f"ORDER DETAILS DEBUG - Order #{order.id}")
    print(f"Order Status: {order.product_status}")
    print(f"Order Date: {order.order_date}")
    print(f"Return Status: {order.return_status}")
    print(f"Return Reason: {order.return_reason}")
    print(f"Has Return: {order.has_return}")
    print("=" * 50)
    ...
    # ✅ SIMPLIFIED: Use order-level return status
    now = timezone.now()
    two_days_ago = now - timedelta(days=2)
    
    # ✅ Check if the order has a return
    has_return = order.has_return
    
    print(f"Current Time: {now}")
    print(f"Two Days Ago: {two_days_ago}")
    print(f"Order Date: {order.order_date}")
    print(f"Is Order Date >= Two Days Ago: {order.order_date >= two_days_ago}")
    print(f"Order Status is Delivered: {order.product_status == 'Delivered'}")
    print(f"Has Return: {has_return}")
    
    for item in products_with_details:
        order_item = item['order_item']
        
        # ✅ Check: item is delivered, within 2 days, and order has no return
        can_return_item = (
            order_item.product_status == "Delivered" and
            order.order_date >= two_days_ago and
            not has_return  # ✅ Only eligible if order has no return
        )
        
        print(f"\nItem: {order_item.item}")
        print(f"  Product Status: {order_item.product_status}")
        print(f"  Is Delivered: {order_item.product_status == 'Delivered'}")
        print(f"  Is Within 2 Days: {order.order_date >= two_days_ago}")
        print(f"  No Return: {not has_return}")
        print(f"  Can Return: {can_return_item}")
        
        item['can_return'] = can_return_item
        item['already_returned'] = has_return
        item['return_status'] = order.return_status if has_return else None
    ...
    context = {
        ...
        "order_note": order.order_note,
        "delivery_method_display": order.formatted_delivery_method,
        "kwikpik_tracking": kwikpik_tracking,
        "kwikpik_tracking_data": kwikpik_tracking_data,
        "return_images": return_images,
        "has_return_images": return_images.exists(),
    }
    return render(request, "core/order-details.html", context)
...
def contact(request):
    if request.method == "POST":
        name = request.POST.get("name")
        email = request.POST.get("email")
        phone = request.POST.get("phone")  # 🟢 Fixed: was "phone" not "subject"
        message = request.POST.get("message")
        ...
        ContactForm.objects.create(
            name=name,
            email=email,
            subject=phone,  # Using phone as subject
            message=message,
        )
        ...
        send_mail(
            ...
            ''',
            from_email=settings.DEFAULT_FROM_EMAIL,
            recipient_list=['contact.trenva@gmail.com'],
            fail_silently=True,
        )
    ...
    try:
        ...
        with transaction.atomic():
            ...
            _create_user_notification(
                ...
                title='Wallet funded',
                message=f"Your wallet has been credited with ₦{amount:,.2f} via bank transfer.",
            )
...
def mobile_wallet_topup_init(request):
    amount = request.data.get('amount')

    if not amount or float(amount) <= 0 or float(amount) < 100 or float(amount) % 100 != 0:
        return Response(
            {'success': False, 'error': 'Amount must be at least ₦100 and in multiples of 100.'},
            status=400,
        )
    ...
    try:
        ...
        return Response(
            {'success': False, 'error': response_data.get('message', 'Unknown error')},
            status=400,
        )
    except Exception as e:
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def mobile_wallet_topup_verify(request):
    reference = request.data.get('reference')

    if not reference:
        return Response({'success': False, 'error': 'Missing reference.'}, status=400)

    # Idempotency guard — don't double-credit if this reference was already processed
    ...
    try:
        ...
        if not (response_data.get('status') and response_data['data']['status'] == 'success'):
            return Response({'success': False, 'error': 'Transaction verification failed.'}, status=400)

        # Trust Paystack's verified amount, not client input
        amount_decimal = Decimal(str(response_data['data']['amount'])) / Decimal('100')
        wallet = Wallet.objects.get(user=request.user)
...
def vendor_signin(request):
    ...
    if request.method == 'POST':
        username = request.POST.get('username', '').strip()  # Can be email or username
        password = request.POST.get('password', '')
        ...
        # Verify with Google
        data = {
            'secret': '6LdBIxstAAAAACWDgS1ENqFZm0B4WSc5GTeez7TN',
        ...
        try:
            # ✅ STEP 1: Find user by email or username (like OG version)
            if '@' in username:
                user = User.objects.get(email=username)
            else:
                user = User.objects.get(username=username)
            
            # ✅ STEP 2: Check if email is verified (OG version used is_active)
            if not user.is_active:
                messages.error(request, 'Please verify your email address before logging in. Check your email for the verification link.')
                return render(request, 'user/vendor_signin.html')
            ...
            # ✅ STEP 4: Authenticate user (like OG version)
            user = authenticate(request, username=user.username, password=password)
            
            if user is not None:
                # ✅ STEP 5: Check if user is a vendor (NEW - prevents normal users)
                try:
                    vendor = Vendor.objects.get(user=user)
                except Vendor.DoesNotExist:
                ...
                # ✅ STEP 7: Redirect based on vendor approval status
                if vendor.approval_status == 'approved':
                    return redirect('vendor_dashboard')  
                elif vendor.approval_status == 'incomplete':
                    return redirect('vendor_profile_setup')
...
def address_autocomplete(request):
    """Google Places Autocomplete — mirrors the web app's Places widget behavior."""
    query = request.GET.get('q', '').strip()

    if not query or len(query) < 3:
        return JsonResponse({'suggestions': []})

    api_key = settings.GOOGLE_PLACES_SERVER_API_KEY
    if not api_key:
        return JsonResponse({'suggestions': [], 'error': 'Autocomplete not configured'}, status=500)
    ...
    try:
        ...
        if status not in ('OK', 'ZERO_RESULTS'):
            # INVALID_REQUEST, REQUEST_DENIED, OVER_QUERY_LIMIT, UNKNOWN_ERROR
            return JsonResponse({'suggestions': [], 'error': data.get('error_message', status)})
...
def place_details(request):
    """Resolve a Places place_id into a full address + coordinates + components."""
    place_id = request.GET.get('place_id', '').strip()

    if not place_id:
        return JsonResponse({'success': False, 'error': 'place_id is required'}, status=400)

    api_key = settings.GOOGLE_PLACES_SERVER_API_KEY
    if not api_key:
        return JsonResponse({'success': False, 'error': 'Place details not configured'}, status=500)
    ...
    try:
        ...
        if response.status_code != 200:
            return JsonResponse({'success': False, 'error': 'Service unavailable'})

        data = response.json()
        if data.get('status') != 'OK':
            return JsonResponse({'success': False, 'error': data.get('error_message', data.get('status'))})
...
#             # Handle colors with stock - Clear existing and add new
#             if enable_colors and color_ids:
#                 # Reset all color stock to 0
#                 for color in product.color.all():
#                     color.stock = 0
...
#             # Handle sizes with stock - Clear existing and add new
#             if enable_sizes and size_ids:
#                 # Reset all size stock to 0
#                 for size_obj in product.size.all():
#                     size_obj.stock = 0
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['title', 'description', 'sku', 'category__title', 'subcategory__title', 'leveltwocategory__title']
    filterset_fields = ['product_status', 'category', 'subcategory', 'leveltwocategory', 'vendor', 'brand', 'featured', 'in_stock', 'promo']
    ...
    def get_queryset(self):
        ...
        category_title = (params.get('category_title') or params.get('category_name') or '').strip()
        category_cid = (params.get('category_cid') or '').strip()
        subcategory_title = (params.get('subcategory_title') or params.get('subcategory_name') or '').strip()
        leveltwo_title = (params.get('leveltwo_title') or params.get('leveltwo_name') or '').strip()
        vendor_name = (params.get('vendor_name') or '').strip()
        ...
        if category_cid:
            queryset = queryset.filter(category__cid__iexact=category_cid)
        if subcategory_title:
            queryset = queryset.filter(subcategory__title__iexact=subcategory_title)
        if leveltwo_title:
            queryset = queryset.filter(leveltwocategory__title__iexact=leveltwo_title)
        if vendor_name:
            queryset = queryset.filter(vendor__name__iexact=vendor_name)
        if q:
            queryset = queryset.filter(
                ...
                | Q(subcategory__title__icontains=q)
                | Q(leveltwocategory__title__icontains=q)
            )
        if color:
            queryset = queryset.filter(
                Q(color__color_name__icontains=color)
                | Q(title__icontains=color)
        ...
        if sort == 'price_asc':
            queryset = queryset.order_by('price', '-date')
        elif sort == 'price_desc':
            queryset = queryset.order_by('-price', '-date')
        elif sort == 'most_recent':
            queryset = queryset.order_by('-date')
        elif sort == 'popular':
            queryset = queryset.order_by('-average_rating', '-date')
        elif sort == 'top_sales':
            queryset = queryset.order_by('-featured', '-promo', '-date')
    ...
    def related(self, request, pk=None):
        try:
            product = self.get_object()
            queryset = self.get_queryset().filter(product_status='published').exclude(id=product.id)

            if getattr(product, 'leveltwocategory_id', None):
                queryset = queryset.filter(leveltwocategory=product.leveltwocategory)
            elif getattr(product, 'subcategory_id', None):
                queryset = queryset.filter(subcategory=product.subcategory)
            elif getattr(product, 'category_id', None):
...
class SubCategoryViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['title', 'category__title', 'category__cid']
...
class LevelTwoCategoryViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['title', 'category__title', 'category__cid', 'subcategory__title']
...
class VendorViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = Vendor.objects.all().order_by('-date')
    ...
    def approve(self, request, vid=None):
        try:
    ...
    @action(detail=True, methods=['post'])
    def reject(self, request, vid=None):
        try:
            vendor = self.get_object()
    ...
    def follow(self, request, vid=None):
        """
        Mobile/API-friendly follow/unfollow toggle.
        """
        try:
            vendor = self.get_object()

            # Vendor owners cannot follow themselves.
            if getattr(vendor, 'user_id', None) == getattr(request.user, 'id', None):
                return Response(
                    {
                        'success': False,
                        'error': 'You cannot follow your own vendor account.',
                        'is_following': False,
                        'follower_count': vendor.get_follower_count(),
                    },
            ...
            if follow_instance:
                follow_instance.delete()
                is_following = False
                message = f"You unfollowed {vendor.business_name or vendor.store_name or vendor.name}"
            else:
                VendorFollow.objects.create(user=request.user, vendor=vendor)
                is_following = True
                message = f"You are now following {vendor.business_name or vendor.store_name or vendor.name}"
    ...
    @action(detail=True, methods=['get'])
    def products(self, request, vid=None):
        try:
            vendor = self.get_object()
    ...
    @action(detail=True, methods=['get'])
    def orders(self, request, vid=None):
        try:
            vendor = self.get_object()
...
class OrderItemReturnViewSet(viewsets.ModelViewSet):
    """
    ViewSet for order item returns (Customer facing)
    """
    permission_classes = [IsAuthenticated]
    serializer_class = CartOrderItemReturnSerializer
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['product_status', 'return_status', 'order']
    search_fields = ['item', 'order__oid']
    ordering_fields = ['order_date', 'return_status']
    ordering = ['-order_date']
    
    def get_queryset(self):
        user = self.request.user
        if user.is_staff:
            return CartOrderItems.objects.all().select_related('order', 'user', 'product_ref').prefetch_related('return_images')
        return CartOrderItems.objects.filter(user=user).select_related('order', 'product_ref').prefetch_related('return_images')
    
    @action(detail=True, methods=['post'])
    def request_return(self, request, pk=None):
        """
        Request a return for a specific order item
        POST /api/order-items/{id}/request-return/
        """
        try:
            ...
            has_return = item.return_reason is not None and item.return_reason != ''
            
            if item.product_status != "Delivered":
                return Response(
                    {'error': 'Item must be delivered before returning', 
                     'current_status': item.product_status}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
            
            if order.order_date < two_days_ago:
                return Response(
                    {'error': f'Return period has expired (2 days from delivery). Order was placed on {order.order_date.strftime("%Y-%m-%d")}', 
                     'order_date': order.order_date, 
                     'grace_period_days': 2}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            # Update item
            item.return_reason_category = validated_data['return_reason_category']
            item.return_reason = validated_data.get('return_reason', '')
            item.return_status = 'pending'
            item.refund_preference = validated_data.get('refund_preference', 'wallet')
            item.save()
            ...
            for image in images[:5]:
                img = ReturnItemImage.objects.create(
                    return_item=item,
                    image=image
                )
                saved_images.append(img)
            
            # Check if all items in order have returns
            all_items = CartOrderItems.objects.filter(order=order)
            all_returned = all(
                getattr(i, 'return_reason', None) is not None and i.return_reason != ''
                for i in all_items
            )
            if all_returned:
                order.product_status = "Return Requested"
                order.save()
            ...
            try:
                _create_user_notification(
                    ...
                    title=f'Return Requested: {item.item}',
                    message=f'Your return request for {item.item} has been submitted. We\'ll review it shortly.',
                    order=order,
                    order_item=item
                )
                ...
                for admin in admin_users:
                    _create_user_notification(
                        user=admin,
                        notification_type='return_request',
                        title=f'New Return Request: {item.item}',
                        message=f'User {request.user.username} requested return for {item.item} (Order #{order.oid})',
                        order=order,
                        order_item=item
                    )
            except Exception as e:
                logger.error(f"Failed to send return notifications: {str(e)}")
            
            # Return response
            response_data = CartOrderItemReturnSerializer(item, context={'request': request}).data
            response_data['saved_images'] = len(saved_images)
            
            return Response({
                'success': True,
                'message': f'Return requested for {item.item}',
                'data': response_data
            }, status=status.HTTP_200_OK)
            ...
            logger.error(f"Error in request_return: {str(e)}")
            return Response(
                {'error': str(e)}, 
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def my_returns(self, request):
        ...
        GET /api/order-items/my-returns/
        """
        items = CartOrderItems.objects.filter(
            user=request.user,
            return_reason__isnull=False
        ).exclude(return_reason='').order_by('-order_date')
        
        page = self.paginate_queryset(items)
        if page is not None:
            serializer = CartOrderItemReturnSerializer(page, many=True, context={'request': request})
            return self.get_paginated_response(serializer.data)
        
        serializer = CartOrderItemReturnSerializer(items, many=True, context={'request': request})
        return Response(serializer.data)
    
    @action(detail=False, methods=['get'])
    def summary(self, request):
        """
        Get return summary for the current user
        ...
        total_returns = items.count()
        pending_count = items.filter(return_status='pending').count()
        approved_count = items.filter(return_status='approved').count()
        rejected_count = items.filter(return_status='rejected').count()
        refunded_count = items.filter(return_status='refunded').count()
        
        total_refund_amount = items.filter(
            is_refund_processed=True
        ).aggregate(total=Sum('refund_amount'))['total'] or 0
        
        recent_returns = items.order_by('-order_date')[:5]
        ...
        return Response({
            ...
            'total_refund_amount': float(total_refund_amount),
            'recent_returns': CartOrderItemReturnSerializer(recent_returns, many=True, context={'request': request}).data
        })


class AdminReturnViewSet(viewsets.ModelViewSet):
    """
    Admin ViewSet for managing returns
    """
    permission_classes = [IsAdminUser]
    serializer_class = AdminReturnRequestSerializer
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['return_status', 'product_status', 'vendor']
    search_fields = ['item', 'order__oid', 'user__email', 'user__username']
    ordering_fields = ['order_date', 'return_status']
    ordering = ['-order_date']
    ...
    def get_queryset(self):
        return CartOrderItems.objects.filter(
            return_reason__isnull=False
        ).exclude(return_reason='').select_related(
            'order', 'user', 'product_ref'
        ).prefetch_related('return_images')
    ...
    def process(self, request, pk=None):
        ...
        try:
            ...
            if action == 'approve':
                ...
                try:
                    _create_user_notification(
                        ...
                        title=f'Return Approved: {item.item}',
                        message=f'Your return for {item.item} has been approved. We\'ll process your refund shortly.',
                        order=item.order,
                        order_item=item
                    )
                except Exception as e:
                ...
                return Response({
                    'success': True,
                    'message': f'Return approved for {item.item}',
                    'data': AdminReturnRequestSerializer(item).data
                })
                ...
                try:
                    _create_user_notification(
                        user=item.user,
                        notification_type='order_update',
                        title=f'Return Rejected: {item.item}',
                        message=f'Your return for {item.item} has been rejected. Reason: {admin_notes or "No reason provided"}',
                        order=item.order,
                        order_item=item
                    )
                except Exception as e:
                ...
                return Response({
                    'success': True,
                    'message': f'Return rejected for {item.item}',
                    'data': AdminReturnRequestSerializer(item).data
                })
            
            elif action == 'refund':
                # Process refund
                refund_amount = serializer.validated_data.get('refund_amount', item.price * item.qty)
                ...
                max_refund = item.price * item.qty
                if refund_amount > max_refund:
                    return Response(
                        {'error': f'Refund amount cannot exceed item total (₦{max_refund:,.2f})'}, 
                        status=status.HTTP_400_BAD_REQUEST
                    )
                
                # Process refund to wallet
                wallet, _ = Wallet.objects.get_or_create(user=item.user)
                ...
                Transaction.objects.create(
                    ...
                    transaction_type='credit',
                    status='success',
                    reference=f"REFUND_{item.unique_id}",
                    description=f"Refund for {item.item} from order #{item.order.oid}"
                )
                
                # Update item
                item.return_status = 'refunded'
                item.is_refund_processed = True
                item.refund_amount = refund_amount
                item.refund_date = timezone.now()
                item.admin_notes = admin_notes
                item.save()
                ...
                try:
                    _create_user_notification(
                        user=item.user,
                        notification_type='refund_approved',
                        title=f'Refund Processed: {item.item}',
                        message=f'₦{refund_amount:,.2f} has been refunded to your wallet for {item.item}.',
                        order=item.order,
                        order_item=item
                    )
                except Exception as e:
                ...
                return Response({
                    'success': True,
                    'message': f'Refund of ₦{refund_amount:,.2f} processed for {item.item}',
                    'data': AdminReturnRequestSerializer(item).data
                })
            ...
            logger.error(f"Error in process return: {str(e)}")
            return Response(
                {'error': str(e)}, 
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def stats(self, request):
        ...
        approved = queryset.filter(return_status='approved').count()
        rejected = queryset.filter(return_status='rejected').count()
        refunded = queryset.filter(return_status='refunded').count()
        ...
        pending_refund_amount = queryset.filter(
            return_status='approved',
            is_refund_processed=False
        ).aggregate(total=Sum('price') * Sum('qty'))['total'] or 0
        
        # Return by category
        category_stats = {}
        for choice in CartOrder.RETURN_REASON_CHOICES:
            code, label = choice
            count = queryset.filter(return_reason_category=code).count()
            if count > 0:
        ...
        return Response({
            ...
            'return_rate': round((total / CartOrderItems.objects.count() * 100), 2) if CartOrderItems.objects.count() > 0 else 0
        })
    
    @action(detail=False, methods=['get'])
    def pending(self, request):
        """
        Get only pending returns
        GET /api/admin/returns/pending/
        """
        queryset = self.get_queryset().filter(return_status='pending')
        page = self.paginate_queryset(queryset)
        if page is not None:
            serializer = self.get_serializer(page, many=True) 
            return self.get_paginated_response(serializer.data)
...
#     # ✅ ADD THIS METHOD TO PASS REQUEST CONTEXT
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    # ✅ ADD THIS - Fix image URLs in list view
        ...
        for order in response.data.get('results', []):
            for item in order.get('items', []):
                if 'image' in item and item['image']:
                    if '/media/media/' in item['image']:
                        item['image'] = item['image'].replace('/media/media/', '/media/')
                if 'image_url' in item and item['image_url']:
                    if '/media/media/' in item['image_url']:
                        item['image_url'] = item['image_url'].replace('/media/media/', '/media/')
        
        return response
    
    # ✅ ADD THIS - Fix image URLs in single order view
    def retrieve(self, request, *args, **kwargs):
        response = super().retrieve(request, *args, **kwargs)
        
        # Fix image URLs for single order
        order = response.data
        for item in order.get('items', []):
            if 'image' in item and item['image']:
                if '/media/media/' in item['image']:
...
class CouponViewSet(viewsets.ModelViewSet):
    queryset = Coupon.objects.all().prefetch_related("product", "specific_users").order_by("-id")
    ...
    # def get_queryset(self):
    #     qs = Coupon.objects.all().prefetch_related("product", "specific_users").order_by("-id")
    ...
    #     email = (self.request.data.get("email") if hasattr(self.request, "data") else None) or (user.email if user else None)

    #     # Base validity window
    #     qs = qs.filter(active=True).filter(Q(expiry_date__isnull=True) | Q(expiry_date__gt=now))

    #     # Respect specific_users assignment (show global + assigned)
    #     if user:
    #         qs = qs.filter(Q(specific_users__isnull=True) | Q(specific_users=user)).distinct()
    #     else:
    #         qs = qs.filter(specific_users__isnull=True)

    #     # Exclude usage-limit reached
    #     qs = qs.filter(Q(usage_limit__isnull=True) | Q(usage_count__lt=F("usage_limit")))

    #     # Exclude already-used-by-email
    #     if email:
    #         qs = qs.exclude(couponemail__user_email=email)
    ...
    def get_queryset(self):
        ...
        # Show coupons user is allowed to see (global + assigned)
        if user:
            qs = qs.filter(Q(specific_users__isnull=True) | Q(specific_users=user)).distinct()
        else:
            qs = qs.filter(specific_users__isnull=True)
    ...
    def validate(self, request):
        ...
        try:
            ...
            coupon = (
                Coupon.objects.filter(coupon_code__iexact=coupon_code, active=True)
                .order_by("-id")
                .first()
            )
        ...
        if user and coupon.specific_users.exists() and not coupon.specific_users.filter(id=user.id).exists():
            return Response(
                {"valid": False, "message": "This coupon is not valid for your account."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def use(self, request, pk=None):
        try:
            ...
            # Increment only by global usage limit rule
            Coupon.objects.filter(id=coupon.id).update(usage_count=F("usage_count") + 1)
    
            # Keep history record (optional, does NOT block reuse)
            if email:
                CouponEmail.objects.create(coupon=coupon, user_email=email)
            ...
            return Response(
                {
                    ...
                    "remaining_uses": (
                        ...
                        else max(0, coupon.usage_limit - coupon.usage_count)
                    ),
                },
                status=status.HTTP_200_OK,
            )
...
class ContactFormViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # Send support notification email after saving contact message
        support_email = getattr(settings, "SUPPORT_EMAIL", "contact.trenva@gmail.com")
        subject = f"[Customer Support] {contact.subject or 'New message'}"
        body = (
            "A new customer support message was submitted.\n\n"
            f"Name: {contact.name or 'N/A'}\n"
            f"Email: {contact.email or 'N/A'}\n"
            f"Subject: {contact.subject or 'N/A'}\n\n"
            "Message:\n"
            f"{contact.message or ''}\n"
        )
        ...
        try:
            ...
            logger.error(f"Failed to send support contact email: {str(mail_error)}")
...
class MobileTokenObtainPairSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        if user.is_superuser or user.is_staff:
            raise AuthenticationFailed("This login is only for regular customers. Vendor and admin accounts are not allowed.")

        if Vendor.objects.filter(user=user).exists():
            raise AuthenticationFailed("This login is only for regular customers. Vendor accounts are not allowed.")

        if hasattr(user, "email_confirmed") and not bool(user.email_confirmed):
...
def mobile_google_login(request):
    """
    Mobile-only OAuth entrypoint.
    Seeds callback intent into session, then starts allauth Google login.
    """
    from django.shortcuts import redirect
    from urllib.parse import quote

    mobile_callback = request.GET.get("mobile_callback") or "trenva:///auth-callback"
    request.session["mobile_callback"] = mobile_callback
    request.session["is_mobile_login"] = True
    request.session.modified = True

    next_path = "/ng/api/auth/mobile-social-complete/"
    return redirect(f"/ng/accounts/google/login/?process=login&next={quote(next_path, safe='/?=&')}")

    
@api_view(['GET'])
@permission_classes([AllowAny])
def mobile_social_complete(request):
    from urllib.parse import quote
    from django.http import HttpResponse
    from django.shortcuts import redirect
    ...
    mobile_callback = (
        request.session.pop("mobile_callback", None)
        or request.GET.get("mobile_callback")
        or "trenva:///auth-callback"
    )
    request.session.pop("is_mobile_login", None)
    request.session.modified = True
    ...
    if not request.user.is_authenticated:
        ...
        request.session["is_mobile_login"] = True
        request.session.modified = True
        encoded_callback = quote(mobile_callback, safe="")
        return redirect(
            f"/ng/accounts/google/login/?process=login"
            f"&next={quote('/ng/api/auth/mobile-social-complete/?mobile_callback=' + encoded_callback, safe='')}"
        )
    ...
    separator = "&" if "?" in mobile_callback else "?"
    deep_link = (
        f"{mobile_callback}{separator}"
        f"access={quote(access_token, safe='')}&"
        f"refresh={quote(refresh_token, safe='')}"
    )

    # Use HTML + JS redirect instead of 302 — works on both iOS Safari and Android
    html = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>Redirecting to Trenva...</title>
        <style>
            body {{
    ...
    <body>
        ...
        <p>Returning to Trenva...</p>
        <a href="{deep_link}">Tap here if not redirected</a>
        <script>
            // Try immediately
            window.location.href = "{deep_link}";
            // Fallback after short delay in case the first attempt is blocked
            setTimeout(function() {{
                window.location.href = "{deep_link}";
            }}, 500);
        </script>
...
def mobile_register(request):
    try:
        ...
        user = User.objects.create(
            ...
            last_name=last_name,
            # Keep active like web signup; enforce verification via email_confirmed.
            is_active=True,
        ...
        # Wallet is auto-created by User post_save signal in models.py.
        # Keep this idempotent in case signal behavior changes.
        ...
        try:
            ...
            token = signer.sign(email.lower())
            verify_url = request.build_absolute_uri(
                f"{reverse('verify_email')}?token={token}"
            )

            subject = "Verify your Trenva account"
            email_context = {
                "user": user,
                "username": username,
                "full_name": full_name,
            ...
            html_message = render_to_string("emails/user_email_confirm.html", email_context)
            ...
            logger.error(f"Failed to send mobile registration email to {email}: {str(mail_error)}")
...
@api_view(['GET'])
@permission_classes([AllowAny])
def mobile_verify_email(request, uidb64, token):
    from django.utils.http import urlsafe_base64_decode
    ...
    deep_link = "trenva:///auth-callback?verified=1"
    html = f"""
    <!doctype html>
    <html>
      <head>
        ...
        <style>
          body {{ font-family: Arial, sans-serif; background:#fff; color:#111; padding:24px; text-align:center; }}
          .btn {{ display:inline-block; margin-top:16px; background:#ff6200; color:#fff; text-decoration:none; padding:12px 16px; border-radius:10px; font-weight:600; }}
          .hint {{ color:#555; font-size:14px; margin-top:12px; }}
        </style>
      </head>
      <body>
        <h2>Email verified successfully</h2>
        <p>Opening Trenva app...</p>
        <a class="btn" href="{deep_link}">Open Trenva App</a>
...
#                 # ========== SEND RETURN INITIATION EMAIL ==========
#                 try:
#                     # Get order items
#                     order_items = CartOrderItems.objects.filter(order=order)
                    
#                     # Pickup station details (you can make this dynamic from DB)
#                     pickup_station = {
#                         'name': 'Trenva Pickup Station',
...
#                         'return_reason': dict(order.RETURN_REASON_CHOICES).get(return_reason, return_reason),
#                     }
                    
#                     # Render email template
#                     html_message = render_to_string('emails/return_initiation.html', context)
                    
#                     # Send email
#                     email = EmailMessage(
#                         subject=f'Return Request Initiated - Order #{order.oid}',
#                         body=html_message,
#                         from_email=settings.DEFAULT_FROM_EMAIL,
#                         to=[request.user.email],
#                     )
...
#                     logger.info(f"Return initiation email sent to {request.user.email} for order {order.oid}")
                    
#                 except Exception as e:
#                     logger.error(f"Failed to send return initiation email: {str(e)}")
#                 # ========== END EMAIL ==========
...
def request_return(request, order_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            return_description = request.POST.get('return_description')
            return_images = request.FILES.getlist('return_images')
            refund_preference = request.POST.get('refund_preference', 'wallet')
            
            if not return_reason:
                messages.error(request, "Please select a return reason.")
                return render(request, 'core/request_return.html', {
                    'order': order, 
                    'return_reasons': order.RETURN_REASON_CHOICES
                })
            
            if len(return_images) < 2:
                messages.error(request, "Please upload 2 images of the product.")
                return render(request, 'core/request_return.html', {
                    'order': order, 
                    'return_reasons': order.RETURN_REASON_CHOICES
            ...
            for idx, image in enumerate(return_images[:2]):
                try:
                    # Check file size (max 5MB)
                    if image.size > 5 * 1024 * 1024:
                        image_errors.append(f"Image '{image.name}' is too large (max 5MB)")
                        continue
                    
                    # Open and resize image with proper error handling
                    try:
                        img = Image.open(image)
                    except Exception as e:
                        image_errors.append(f"Cannot read image '{image.name}': {str(e)}")
                        continue
                    ...
                    # Save to BytesIO with high compression
                    img_io = BytesIO()
                    img.save(img_io, format='JPEG', quality=70, optimize=True)
                    img_io.seek(0)
                    
                    # Create file name
                    image_name = f"return_{order.id}_{idx+1}.jpg"
                    resized_image = ContentFile(img_io.getvalue(), name=image_name)
                    saved_images.append(resized_image)
                    
                except MemoryError:
                    image_errors.append(f"Image '{image.name}' is too large to process")
                    continue
                except Exception as e:
                    image_errors.append(f"Error processing '{image.name}': {str(e)}")
                    continue
            
            # Check if we have at least 2 images saved
            if len(saved_images) < 2:
                error_msg = "Could not process images. " + " ".join(image_errors[:2])
            ...
            for resized_image in saved_images[:2]:
                ReturnImage.objects.create(
            ...
            order.return_reason = return_description or f"Return requested: {dict(order.RETURN_REASON_CHOICES).get(return_reason, return_reason)}"
            order.product_status = "Return Requested"
            order.refund_preference = refund_preference
            order.save()
...
def pickup_station_receive_return(request, order_id):
    """
    View for pickup station/admin to mark a return as received
    """
    from django.core.mail import EmailMessage
    from django.template.loader import render_to_string
    from django.conf import settings
    ...
    try:
        ...
        if not order.return_reason_category:
            messages.error(request, "No return request found for this order.")
            return redirect('admin_dashboard')
        
        # Check if already received
        if order.product_status == "Return Received":
            messages.warning(request, "This return has already been marked as received.")
            return redirect('admin_dashboard')
        
        # Mark as received
        order.product_status = "Return Received"
        order.save()
        ...
        try:
            ...
            html_message = render_to_string('emails/return_received.html', context)
            
            email = EmailMessage(
                subject=f'Return Received - Order #{order.oid}',
                body=html_message,
                from_email=settings.DEFAULT_FROM_EMAIL,
                to=[order.user.email],
            )
            ...
            logger.info(f"Return received email sent to {order.user.email} for order {order.oid}")
            
        except Exception as e:
            logger.error(f"Failed to send return received email: {str(e)}")
        # ========== END EMAIL ==========
        
        messages.success(request, f"Return for order #{order.oid} has been marked as received. Customer has been notified.")
        return redirect('admin_dashboard')
        ...
        logger.error(f"Error in pickup_station_receive_return: {str(e)}")
        messages.error(request, f"An error occurred: {str(e)}")
        return redirect('admin_dashboard')
...
def update_order_status(request, order_item_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if new_status != old_status and order_item.order and order_item.order.user:
                _create_user_notification(
                    user=order_item.order.user,
                    notification_type='order_update',
                    title='Order status updated',
                    message=f"Order #{order_item.order.oid}: {order_item.item} changed from {old_status} to {new_status}.",
...
#     """
#     View for vendors to reject an order item - NO FORMS
#     """
#     try:
#         vendor = Vendor.objects.get(user=request.user)
...
#         if order_item.product_status == 'Rejected by Vendor':
#             messages.error(request, "This order item has already been rejected.")
#             return redirect('vendor_orders_details', vid=vendor.vid, unique_id=unique_id)
        
#         # Check if order can be rejected
#         rejectable_statuses = ['Placed', 'Confirmed']
#         if order_item.product_status not in rejectable_statuses:
#             messages.error(request, f"This order is {order_item.product_status} and cannot be rejected.")
#             return redirect('vendor_orders_details', vid=vendor.vid, unique_id=unique_id)
        
#         # Check for too many recent rejections
#         recent_rejections = OrderRejection.objects.filter(
#             vendor=vendor,
#             rejected_at__gte=timezone.now() - timedelta(days=30)
#         ).count()
...
#                 'REASON_CHOICES': OrderRejection.REASON_CHOICES,
#             }
#             return render(request, 'user/vendor_reject_order.html', context)
...
#                 return redirect('vendor_order_reject', unique_id=unique_id)
            
#             if reason == 'other' and not description:
#                 messages.error(request, "Please provide details when selecting 'Other reasons'.")
#                 return redirect('vendor_order_reject', unique_id=unique_id)
...
#             if recent_rejections >= hard_limit:
#                 messages.error(request, "You have reached the maximum allowed rejections. Please contact admin.")
#                 return redirect('vendor_orders_details', vid=vendor.vid, unique_id=unique_id)
            
#             with transaction.atomic():
#                 # Create rejection record
#                 rejection = OrderRejection.objects.create(
#                     order_item=order_item,
#                     vendor=vendor,
#                     reason=reason,
#                     description=description
...
#                 # Update order item status
#                 order_item.product_status = 'Rejected by Vendor'
#                 order_item.save()
...
#                         title=f'Order Item Rejected - #{order_item.order.oid}',
#                         message=f'Vendor {vendor.name} rejected order item "{order_item.item}". Reason: {rejection.get_reason_display()}',
#                         order=order_item.order,
#                         order_item=order_item
#                     )
                
#                 # Send email to customer
#                 send_rejection_email_to_customer(order_item, rejection)
                
#                 # Send warning email if too many rejections
#                 if recent_rejections >= warning_threshold:
#                     send_warning_email_to_vendor(vendor, recent_rejections + 1)
                
#                 messages.success(request, "Order item rejected successfully. Admin has been notified.")
#                 return redirect('vendor_orders_details', vid=vendor.vid, unique_id=unique_id)
...
#                 order_item=order_item,
#                 vendor=vendor,
#                 reason='other',  # or map to your REASON_CHOICES
...
#                     user=admin_user,
#                     notification_type='order_rejected',
#                     title=f'Order Item Rejected by Vendor',
#                     message=f'Vendor {vendor.name} rejected order item #{order_item.unique_id}. '
...
#             messages.success(request, "Order rejected successfully! Admin has been notified.")
#             return redirect('vendor_orders_details', vid=vendor.vid, unique_id=unique_id)
        
#         else:
#             # GET request - redirect to details page with modal
#             messages.info(request, "Please use the rejection form on the order details page.")
#             return redirect('vendor_orders_details', vid=vendor.vid, unique_id=unique_id)
...
#                 print(f"DEBUG: Payment successful! Amount: {amount}")
...
#                     print(f"DEBUG: Wallet balance updated: {old_balance} -> {float(self.balance)}")
...
# @require_http_methods(["POST"])
# def paystack_webhook(request):
#     """Handle Paystack webhook for virtual account transfers AND admin funding"""
#     try:
#         payload = json.loads(request.body)
#         event = payload.get('event')
#         data = payload.get('data', {})
        
#         print(f"DEBUG: Paystack webhook received event: {event}")
#         print(f"DEBUG: Paystack webhook data: {data}")
...
#             # Check if this is an admin wallet funding
#             if purpose == 'admin_wallet_funding' or reference.startswith('ADM_FUND_'):
#                 print(f"DEBUG: Processing admin wallet funding for reference: {reference}")
#                 admin_wallet = AdminWallet.get_wallet()
                
#                 result = admin_wallet.verify_paystack_payment(reference)
                
#                 if result['success']:
#                     print(f"DEBUG: Successfully processed admin funding: {result}")
#                     # Send email notification to admin
#                     admin_email = metadata.get('admin_email')
#                     if admin_email:
#                         try:
#                             subject = "Admin Wallet Funded Successfully"
#                             message = f"""
#                             Your admin wallet has been successfully funded.
                            
#                             Amount: ₦{result['amount']:,.2f}
#                             Reference: {reference}
#                             Date: {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}
#                             New Balance: ₦{admin_wallet.balance:,.2f}
                            
#                             Thank you for using Trenva.
#                             """
...
#                                 fail_silently=False,
#                             )
#                             print(f"DEBUG: Notification email sent to {admin_email}")
#                         except Exception as e:
#                             print(f"DEBUG: Failed to send email: {str(e)}")
#                 else:
#                     print(f"DEBUG: Failed to process admin funding: {result}")
                
#                 return JsonResponse({'status': 'processed'})
            
#             # Original virtual account logic
#             elif data.get('channel') == 'bank_transfer' and data.get('authorization'):
#                 authorization = data.get('authorization', {})
#                 account_number = authorization.get('sender_bank_account_number')
...
#                             status='success',
#                             reference=reference,
#                             description=f'Bank transfer to {account_number}'
#                         )
                        
#                         print(f"Auto-credited ₦{amount} to {wallet.user.email}")
                        
#                     except Wallet.DoesNotExist:
#                         print(f"Wallet not found for account: {account_number}")
...
#             wallet.account_number = account_number
#             wallet.bank_name = bank_name
#             wallet.virtual_account_id = virtual_account_id
#             wallet.save()
#             logger.info(f"Virtual account assigned for {wallet.user.email}: {account_number}")
#         except Wallet.DoesNotExist:
#             logger.error(f"Wallet not found for customer code: {customer_code}")
    
#     elif event_type == 'dedicatedaccount.assign.failed':
#         # Virtual account creation failed
#         customer_code = data.get('customer', {}).get('customer_code')
#         reason = data.get('reason', 'Unknown')
#         logger.error(f"Virtual account assignment failed for {customer_code}: {reason}")
...
#             wallet.bank_name = bank_name
#             wallet.save()
#             logger.info(f"Virtual account created: {account_number}")
#         except Wallet.DoesNotExist:
#             logger.error(f"Wallet not found for customer: {customer_code}")
    
#     # ✅ NEW: Handle credit events when user receives money
#     elif event_type == 'charge.success':
#         # This is when a user successfully funds their wallet via DVA
...
#     """Process successful charge - credit user's wallet"""
#     try:
#         # Get the charge details
#         amount = Decimal(data.get('amount', 0)) / 100  # Paystack amounts are in kobo
#         reference = data.get('reference')
#         status = data.get('status')
#         customer_email = data.get('customer', {}).get('email')
#         metadata = data.get('metadata', {})
        
#         # Check if this is a DVA funding (you can identify via metadata or other means)
#         # For DVA, you might get the account number from the data
#         account_number = data.get('authorization', {}).get('receiver_bank_account_number')
        
#         logger.info(f"Processing charge success: {reference} - ₦{amount} for {customer_email}")
        
#         # Find the wallet - either by customer email or account number
#         wallet = None
...
#                 wallet = Wallet.objects.get(user=user)
#                 logger.info(f"Found wallet by email: {customer_email}")
#             except (User.DoesNotExist, Wallet.DoesNotExist):
#                 logger.warning(f"Wallet not found for email: {customer_email}")
        
#         # If not found by email, try by account number
#         if not wallet and account_number:
#             try:
#                 wallet = Wallet.objects.get(account_number=account_number)
#                 logger.info(f"Found wallet by account number: {account_number}")
#             except Wallet.DoesNotExist:
#                 logger.warning(f"Wallet not found for account: {account_number}")
        
#         if not wallet:
#             logger.error(f"Could not find wallet for charge: {reference}")
#             return
...
#             status='success',
#             reference=reference,
#             description=f"Wallet funding via bank transfer"
#         )
        
#         logger.info(f"✅ Wallet credited: {wallet.user.email} +₦{amount} (Ref: {reference})")
...
def _mobile_create_kwikpik_delivery(order, address_obj, delivery_method, delivery_fee=0, email=""):
    """
    Create Kwikpik delivery for a paid mobile order and persist tracking fields.
    Mirrors the web place_order Kwikpik behavior for single-vendor and multi-vendor carts.
    """
    try:
        delivery_fee_decimal = Decimal(str(delivery_fee or 0))
    except Exception:
        delivery_fee_decimal = Decimal("0.00")

    normalized_delivery_method = "Door Step Delivery" if delivery_method in [
        "Door Step Delivery",
        "Kwikpik delivery",
        "door_delivery",
    ] else delivery_method

    if normalized_delivery_method != "Door Step Delivery" or delivery_fee_decimal <= 0 or not order.paid_status:
        return

    try:
        all_order_items = list(CartOrderItems.objects.filter(order=order).select_related("product_ref"))
        if not all_order_items:
            logger.warning(f"Kwikpik skipped for mobile order {order.id}: no order items")
            return

        vendor_items_map = {}
        for item in all_order_items:
            vendor_id = item.vendor_id or "1"
            product = getattr(item, "product_ref", None)
            size_category = getattr(product, "size_category", "small") if product else "small"

            if vendor_id not in vendor_items_map:
                vendor_items_map[vendor_id] = []

            vendor_items_map[vendor_id].append({
                "name": (item.item or "Item")[:50],
                "quantity": int(item.qty or 1),
                "total": float(item.total or (item.price * item.qty)),
                "size_category": size_category or "small",
            })
        ...
        base_url = getattr(settings, "KWIKPIK_BASE_URL", "https://logistics-api.kwikpik.io")
        delivery_address = f"{address_obj.address}, {address_obj.city}, {address_obj.state}"
        delivery_name = f"{address_obj.first_name} {address_obj.last_name}".strip() or "Customer"
        delivery_phone = address_obj.phone
        delivery_email = email or order.email_address or getattr(order.user, "email", "")

        if len(unique_vendors) == 1:
            vendor_id = next(iter(unique_vendors))
            vendor_pickup_address = getattr(settings, "KWIKPIK_PICKUP_ADDRESS", "Trenva Warehouse, Lagos")
            vendor_pickup_contact = getattr(settings, "KWIKPIK_PICKUP_CONTACT", "Warehouse Team")
            vendor_pickup_phone = getattr(settings, "KWIKPIK_PICKUP_PHONE", "+2340000000000")
            ...
            unified_order = {
                ...
                "deliveryContactName": delivery_name,
                "deliveryContactPhone": delivery_phone,
                "deliveryContactEmail": delivery_email,
                "description": f"Order #{order.id}",
                "quantity": len(all_order_items),
                "packageValue": float(sum((item.total or 0) for item in all_order_items)),
                "items": [
                    {"name": (item.item or "Item")[:50], "quantity": int(item.qty or 1)}
                    for item in all_order_items
                ],
            }

            if needs_van:
                unified_order["vehicleType"] = "VAN"

            response = requests.post(
                f"{base_url}/api/v2/orders/unified",
                headers=headers,
                json={"orders": [unified_order]},
                timeout=30,
            )
            ...
            if response.status_code in [200, 201]:
                ...
                if successful:
                    order_info = successful[0]
                    package_info = order_info.get("packageInformation", {})
                    tracking_id = package_info.get("trackingId") or package_info.get("trackingID")
                    if tracking_id:
                        order.kwikpik_tracking_id = tracking_id
                        order.kwikpik_tracking_data = {
                            vendor_id: {
                                "tracking_id": tracking_id,
                                "order_code": order_info.get("orderCode"),
                                "status": package_info.get("status", "PENDING"),
                            }
                        }
                        order.save(update_fields=["kwikpik_tracking_id", "kwikpik_tracking_data"])
                        logger.info(f"Kwikpik mobile order created for order {order.id}: {tracking_id}")
                else:
                    failed = data.get("data", {}).get("failed", [])
                    if failed:
                        logger.error(f"Kwikpik mobile unified failed for order {order.id}: {failed[0].get('error')}")
            else:
                logger.error(f"Kwikpik mobile unified API error for order {order.id}: {response.status_code} - {response.text[:200]}")
        ...
        for vendor_id, vendor_items_list in vendor_items_map.items():
            vendor_pickup_address = getattr(settings, "KWIKPIK_PICKUP_ADDRESS", "Trenva Warehouse, Lagos")
            vendor_pickup_contact = getattr(settings, "KWIKPIK_PICKUP_CONTACT", "Warehouse Team")
            vendor_pickup_phone = getattr(settings, "KWIKPIK_PICKUP_PHONE", "+2340000000000")
            vendor_name = "Vendor"
            ...
            order_obj = {
                ...
                "packages": [{
                    ...
                    "deliveryContactEmail": delivery_email,
                    "description": f"Order #{order.id} - {vendor_name}",
                    "packageValue": sum(item.get("total", 0) for item in vendor_items_list),
                    "items": [
        ...
        if response.status_code in [200, 201]:
            ...
            for successful in data.get("data", {}).get("successful", []):
                ...
                tracking_id = packages[0].get("trackingId") or packages[0].get("trackingID")
                if "-V" in order_ref and tracking_id:
                    vendor_id_extracted = order_ref.split("-V")[-1]
                    tracking_data[vendor_id_extracted] = {
                        "tracking_id": tracking_id,
                        "order_code": successful.get("orderCode"),
                        "status": packages[0].get("status", "PENDING"),
                    }

            if tracking_data:
                order.kwikpik_tracking_data = tracking_data
                order.kwikpik_tracking_id = list(tracking_data.values())[0].get("tracking_id")
                order.save(update_fields=["kwikpik_tracking_id", "kwikpik_tracking_data"])
                logger.info(f"Kwikpik mobile bulk order created for order {order.id}: {len(tracking_data)} vendors")

            for failed in data.get("data", {}).get("failed", []):
                logger.error(f"Kwikpik mobile bulk failed for order {order.id}: {failed.get('error')}")
        else:
            logger.error(f"Kwikpik mobile bulk API error for order {order.id}: {response.status_code} - {response.text[:200]}")

    except Exception as e:
        logger.error(f"Kwikpik mobile error for order {order.id}: {str(e)}", exc_info=True)
...
def mobile_paystack_init(request):
    ...
    delivery_fee = kwikpik.calculate_delivery_fee(address_obj.state)
    final_total = final_total + Decimal(str(delivery_fee))
...
def mobile_paystack_verify(request):
    ...
    try:
        ...
        ctx_delivery_fee = Decimal(str(ctx.get("delivery_fee", "0")))
        ...
        _mobile_create_kwikpik_delivery(
            order=order,
            address_obj=address_obj,
            delivery_method=ctx.get("delivery_method", "Door Step Delivery"),
            delivery_fee=ctx_delivery_fee,
            email=user.email,
        )
        ...
        _create_user_notification(
            ...
            notification_type='order_placed',
            title='Order placed successfully',
            message=f"Your order #{order.oid} has been placed and payment was confirmed.",
            order=order,
        )
        ...
        return Response(
            {
                ...
                "order_id": order.id,
                "oid": order.oid,
                "kwikpik_tracking_id": order.kwikpik_tracking_id,
                "kwikpik_tracking_data": order.kwikpik_tracking_data,
    ...
    try:
        with transaction.atomic():
            ...
            _create_user_notification(
                ...
                message=f"{final_total} was debited from your wallet for order #{order.oid}.",
                order=order,
            )
            _create_user_notification(
                user=user,
                notification_type='order_placed',
                title='Order placed successfully',
                message=f"Your order #{order.oid} has been placed using wallet payment.",
                order=order,
            )
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def register_mobile_push_token(request):
    token = str(request.data.get("token", "")).strip()
    try:
        logger.info(
            f"[push-register] hit user_id={getattr(request.user, 'id', None)} "
            f"auth={bool(getattr(request.user, 'is_authenticated', False))} "
            f"platform={request.data.get('platform')} app_version={request.data.get('app_version')} "
            f"token_prefix={token[:24] if token else '<empty>'}"
        )
    except Exception:
        pass
    if not token:
        logger.warning("[push-register] rejected: missing token")
        return Response({"success": False, "error": "token is required"}, status=status.HTTP_400_BAD_REQUEST)

    # Expo token prefix differs by runtime/version:
    # - ExponentPushToken[...] (common in Expo Go / older flows)
    # - ExpoPushToken[...] (newer dev/standalone builds)
    if not (token.startswith("ExponentPushToken[") or token.startswith("ExpoPushToken[")):
        logger.warning("[push-register] rejected: invalid token format")
        return Response({"success": False, "error": "invalid expo push token"}, status=status.HTTP_400_BAD_REQUEST)

    platform = str(request.data.get("platform", "")).strip() or None
    device_name = str(request.data.get("device_name", "")).strip() or None
    app_version = str(request.data.get("app_version", "")).strip() or None

    from core.models import MobilePushToken
    ...
    # Keep only one active token record for same user+token pair and reactivate token on reuse.
    MobilePushToken.objects.filter(user=request.user, token=token).update(is_active=True)
    logger.info(f"[push-register] saved token_id={obj.id} created={created} user_id={getattr(request.user, 'id', None)}")
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def unregister_mobile_push_token(request):
    token = str(request.data.get("token", "")).strip()
    if not token:
        return Response({"success": False, "error": "token is required"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    MobilePushToken.objects.filter(user=request.user, token=token).update(is_active=False)
...
def send_test_push_notification(request):
    ...
    title = str(request.data.get("title", "Trenva Test Notification")).strip() or "Trenva Test Notification"
    body = str(request.data.get("body", "Push notifications are enabled on this device.")).strip() or "Push notifications are enabled on this device."
    send_push_to_user(request.user, title=title, body=body, data={"type": "test"})
    return Response({"success": True, "message": "Test notification sent"})
    
    
def send_refund_processed_email(order, customer, refund_amount):
    try:
        from django.core.mail import EmailMessage
        from django.template.loader import render_to_string
        from django.conf import settings
        ...
        email = EmailMessage(
            subject=f'Your refund for Order #{order.oid} has been processed!',
            body=html_message,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[customer.email],
        )
        ...
        logger.info(f"Refund processed email sent to {customer.email} for order {order.oid}")
        return True
        
    except Exception as e:
        logger.error(f"Failed to send refund processed email: {str(e)}")
        return False
...
#     """
#     Admin view to process a refund for a returned order
#     """
#     try:
#         order = CartOrder.objects.get(id=order_id, product_status="Return Received")
        
#         # Calculate refund amount
#         order_items = CartOrderItems.objects.filter(order=order)
#         refund_amount = sum(item.total for item in order_items)
...
#         order.is_refund_processed = True
#         order.refund_amount = refund_amount
#         order.refund_date = timezone.now()
#         order.product_status = "Refunded"
#         order.save()
        
#         # ========== SEND REFUND EMAIL ==========
#         send_refund_processed_email(order, order.user, refund_amount)
#         # ========== END EMAIL ==========
        
#         messages.success(request, f"Refund of ₦{refund_amount} processed for order #{order.oid}")
#         return redirect('admin_dashboard')
...
def process_refund(request, order_id):
    try:
        ...
        # ========== CHECK REFUND PREFERENCE ==========
        if order.refund_preference == 'voucher':
            # Generate voucher code
            import shortuuid
            voucher_code = f"VOUCHER-{shortuuid.uuid()[:8].upper()}"
            
            # Create voucher/credit record (you'll need a Voucher model)
            ...
            # Mark order as refunded with voucher
            order.is_refund_processed = True
            order.refund_amount = refund_amount
            order.refund_date = timezone.now()
            order.product_status = "Refunded (Voucher)"
            order.save()
            
            # Send voucher email (different from wallet email)
            # send_voucher_email(order, order.user, refund_amount, voucher_code)
            
            messages.success(request, f"Voucher of ₦{refund_amount} created for order #{order.oid}")
            ...
            messages.success(request, f"Refund of ₦{refund_amount} processed to wallet for order #{order.oid}")
...
def send_return_pickup_email(order, customer):
    """
    Send email when rider picks up the return item
    """
    try:
        from django.core.mail import EmailMessage
        from django.template.loader import render_to_string
        ...
        html_message = render_to_string('emails/return_pickup.html', context)
        
        email = EmailMessage(
            subject=f'Your Trenva return is on its way! 🚚',
            body=html_message,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[customer.email],
        )
        ...
        logger.info(f"Return pickup email sent to {customer.email} for order {order.oid}")
        return True
        
    except Exception as e:
        logger.error(f"Failed to send return pickup email: {str(e)}")
        return False
...
@login_required
@user_passes_test(lambda u: u.is_staff)
def rider_picked_up_return(request, order_id):
    """
    View for admin/rider to confirm pickup of return item
    """
    try:
        order = CartOrder.objects.get(id=order_id)
        ...
        if order.product_status == "Picked Up":
            messages.warning(request, "This return has already been picked up.")
            return redirect('admin_dashboard')
        
        # Mark as picked up
        order.product_status = "Picked Up"
        order.save()
        ...
        try:
            ...
            logger.info(f"Return pickup email sent to {order.user.email} for order {order.oid}")
            
        except Exception as e:
            logger.error(f"Failed to send return pickup email: {str(e)}")
        # ========== END EMAIL ==========
        
        messages.success(request, f"Return for order #{order.oid} has been marked as picked up. Customer has been notified.")
        return redirect('admin_dashboard')
        ...
        logger.error(f"Error in rider_picked_up_return: {str(e)}")
        messages.error(request, f"An error occurred: {str(e)}")
        return redirect('admin_dashboard')
...
@user_required
def request_item_return(request, order_item_id):
    """
    View for customers to request a return for a specific item in a multi-item order
    """
    from admin_dashboard.signals import notify_return_request
    from django.core.mail import EmailMessage
    from django.template.loader import render_to_string
    ...
    # Use the model's choices
    from core.models import CartOrder, REFUND_PREFERENCE_CHOICES
    
    try:
        # Get the specific order item
        order_item = CartOrderItems.objects.get(id=order_item_id, user=request.user)
        order = order_item.order
        
        # Check if item is eligible for return
        # ✅ FIXED: Check if there's actually a return reason
        return_reason = getattr(order_item, 'return_reason', None)
        return_status = getattr(order_item, 'return_status', None)
        now = timezone.now()
        two_days_ago = now - timedelta(days=2)
        
        # Only consider it a return if there's actually a return reason
        has_return = return_reason is not None and return_reason != ''
        
        can_return = (
            order_item.product_status == "Delivered" and
            order.order_date >= two_days_ago and
            not has_return  # ✅ Only eligible if no return exists
        )
        ...
        if request.method == 'POST':
            ...
            if not return_reason:
                messages.error(request, "Please select a return reason.")
                return render(request, 'core/request_item_return.html', {
                    'order_item': order_item,
                    'order': order,
                    'return_reasons': order.RETURN_REASON_CHOICES,  # ✅ Use model's choices
                    'refund_preferences': REFUND_PREFERENCE_CHOICES,
                })
            ...
            for idx, image in enumerate(return_images[:2]):
                try:
                    ...
                    # Create file name
                    image_name = f"return_item_{order_item.id}_{idx+1}.jpg"
                    resized_image = ContentFile(img_io.getvalue(), name=image_name)
                    saved_images.append(resized_image)
            ...
            # ========== UPDATE ORDER ITEM WITH RETURN INFO ==========
            order_item.return_reason_category = return_reason
            order_item.return_reason = return_description or f"Return requested: {dict(order.RETURN_REASON_CHOICES).get(return_reason, return_reason)}"
            order_item.return_status = 'pending'
            order_item.refund_preference = refund_preference
            order_item.save()
            
            # ========== SAVE RETURN IMAGES ==========
            from core.models import ReturnItemImage
            
            for resized_image in saved_images[:2]:
                ReturnItemImage.objects.create(
                    return_item=order_item,
                    image=resized_image
                )
            
            # ========== UPDATE ORDER STATUS (if needed) ==========
            # Check if all items in order have return status
            all_items = CartOrderItems.objects.filter(order=order)
            all_returned = all(
                getattr(item, 'return_reason', None) is not None and item.return_reason != ''
            ...
            try:
                ...
                context = {
                    ...
                    'quantity': order_item.qty,
                    'refund_preference': dict(REFUND_PREFERENCE_CHOICES).get(refund_preference, 'Wallet'),
                }
                
                # Render email template
                html_message = render_to_string('emails/return_item_initiation.html', context)
                
                # Send email
                email = EmailMessage(
                    subject=f'Return Request Initiated for {order_item.item} - Order #{order.oid}',
                    body=html_message,
                    from_email=settings.DEFAULT_FROM_EMAIL,
                    to=[request.user.email],
                )
                ...
                logger.info(f"Return item initiation email sent to {request.user.email} for order {order.oid}")
                
            except Exception as e:
                logger.error(f"Failed to send return item initiation email: {str(e)}")
            ...
            try:
                ...
                for admin in admin_users:
                    Notification.objects.create(
                        user=admin,
                        notification_type='return_request',
                        title=f'Return Request for {order_item.item}',
                        message=f'User {request.user.username} requested return for item "{order_item.item}" in order #{order.oid}. Reason: {return_reason}',
                        order=order,
                        order_item=order_item,
                    )
            except Exception as e:
            ...
            messages.success(request, f"Return request submitted for {order_item.item}! We'll review your request and contact you soon.")
            return redirect('order-details', id=order.id)
        ...
        return render(request, 'core/request_item_return.html', context)
        
    except CartOrderItems.DoesNotExist:
        messages.error(request, "Item not found.")
        return redirect('orders')
    except Exception as e:
        logger.error(f"Error in request_item_return: {str(e)}")
        messages.error(request, f"An error occurred: {str(e)}")
        return redirect('order-details', id=order.id)