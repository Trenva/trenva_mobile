...
#     # REMOVED: color and size ManyToMany fields (now handled by variations)
#     # REMOVED: stock_count field (now handled by variations)
#     # REMOVED: in_stock field (now handled by variations)
...
#     # Simplified size category field
#     size_category = models.CharField(
#         max_length=20,
#         choices=WEIGHT_CATEGORIES,
#         default='small',
...
#         return self.variations.aggregate(total=models.Sum('stock_count'))['total'] or 0
    
#     def has_stock(self):
#         """Check if any variation has stock"""
#         return self.variations.filter(stock_count__gt=0).exists()
    
#     def get_variations_by_attribute(self, attribute_name):
#         """Get all variations grouped by attribute value"""
#         variations_by_value = {}
#         for variation in self.variations.filter(is_active=True):
#             attr_value = variation.attributes.get(attribute_name, 'Default')
#             if attr_value not in variations_by_value:
#                 variations_by_value[attr_value] = []
#             variations_by_value[attr_value].append(variation)
#         return variations_by_value
...
#     """Individual variation of a product (e.g., Red Shirt in Size Large)"""
    
#     vid = ShortUUIDField(unique=True, length=10, max_length=20, alphabet="abcdefgTrenva")
...
#     # Stock management
#     stock_count = models.PositiveIntegerField(default=0, help_text="Quantity available for this variation")
    
#     # Price override (if this variation costs different from base product)
#     price_override = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text="Leave blank to use product price")
    
#     # Variation attributes stored as JSON (e.g., {"color": "Red", "size": "Large"})
#     attributes = models.JSONField(default=dict, help_text="Example: {'color': 'Red', 'size': 'Large'}")
    
#     # Variation-specific image (optional)
#     image = models.ImageField(upload_to="variation_images", null=True, blank=True)
    
#     # Status
#     is_active = models.BooleanField(default=True, help_text="Inactive variations won't show in store")
...
#         unique_together = ['product', 'sku']  # Prevent duplicate SKUs per product
    
#     def __str__(self):
#         attr_str = ', '.join([f"{k}: {v}" for k, v in self.attributes.items()])
#         return f"{self.product.title} - {attr_str}"
    
#     @property
#     def effective_price(self):
#         """Return override price if set, otherwise product price"""
...
#     def increase_stock(self, quantity=1):
#         """Increase stock when restocked or refunded"""
#         self.stock_count += quantity
#         self.save()
#         return True
    
#     def can_order(self, quantity=1):
#         """Check if this variation can be ordered"""
#         return self.is_active and self.stock_count >= quantity
    
#     def get_display_name(self):
#         """Get variation display name (e.g., 'Red / Large')"""
#         return ' / '.join(self.attributes.values()) if self.attributes else "Default"
...
REFUND_PREFERENCE_CHOICES = (
    ('wallet', 'Refund to Trenva Wallet'),
...
class CartOrder(models.Model):
    ...
    kwikpik_tracking_id = models.CharField(max_length=100, blank=True, null=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    first_name = models.CharField(max_length=500, null=True)
    last_name = models.CharField(max_length=500, null=True)
    company_name = models.CharField(max_length=500, null=True)
    ...
    refund_preference = models.CharField(
        ...
        blank=True,
        null=True,
        verbose_name="Refund Preference"
    )
    ...
    coupon_used = models.BooleanField(default=False)
    session_token = models.CharField(max_length=100, unique=True, null=True, blank=True)
    kwikpik_tracking_data = models.JSONField(null=True, blank=True, default=dict, help_text="Store multiple vendor tracking IDs")
    
    # ========== RETURN FIELDS (Order Level) ==========
    return_reason = models.TextField(blank=True, null=True, verbose_name="Return Reason")
    return_reason_category = models.CharField(
        max_length=50, 
        choices=RETURN_REASON_CHOICES, 
    ...
    return_status = models.CharField(
        ...
        default='none',  # ✅ Default is 'none' - no return
    ...
    @property
    def can_return(self):
        """Check if the entire order can be returned"""
        if self.product_status != "Delivered":
            return False
        from django.utils import timezone
        from datetime import timedelta
    ...
    # @property
    # def has_return(self):
    #     """Check if the order has a return request"""
    #     return self.return_status not in ['none', None] and self.return_reason is not None and self.return_reason != ''
    
    @property
    def has_return(self):
        """Check if the order has a return request"""
        # Return True only if there's actually a return reason and status is not 'none'
        return (
            self.return_status not in ['none', None] and 
            self.return_reason is not None and 
            self.return_reason != ''
        )
...
class CartOrderItems(models.Model):
    ...
    return_reason_category = models.CharField(
        max_length=50,
        choices=CartOrder.RETURN_REASON_CHOICES,
        blank=True,
        null=True,
        verbose_name="Return Reason Category"
    )
    return_status = models.CharField(
        ...
        blank=True,
        null=True,  # ← IMPORTANT: Allow null
        default=None,  # ← IMPORTANT: No default
        verbose_name="Return Status"
    )
    refund_amount = models.DecimalField(
        max_digits=10, 
    ...
    is_refund_processed = models.BooleanField(
        default=False,
        verbose_name="Refund Processed"
    )
    ...
    def __str__(self):
        return f"{self.item} - #{self.invoice_no} - {self.product_status}"


class ReturnItemImage(models.Model):
    """Images for item-level returns"""
    return_item = models.ForeignKey('CartOrderItems', on_delete=models.CASCADE, related_name='return_images')
    image = models.ImageField(upload_to='return-item-images/')
    uploaded_at = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return f"Return image for {self.return_item.item}"
    
    class Meta:
        verbose_name_plural = "Return Item Images"
            ...
            for bank in banks:
                ...
                try:
                    ...
                    if response_data.get('status'):
                        ...
                        account_data = response_data['data']
                        self.account_number = account_data['account_number']
                        self.account_name = account_data.get('account_name') or self.account_name
                        self.bank_name = account_data['bank']['name']
                        self.virtual_account_id = account_data.get('id', '')
                        self.save(update_fields=['account_number', 'account_name', 'bank_name', 'virtual_account_id'])
...
class MobilePushToken(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="mobile_push_tokens")
...
def send_mobile_push_for_notification(sender, instance, created, **kwargs):
    if not created:
        return
    try:
        from core.push_notifications import send_push_to_user
        payload = {"notification_id": instance.id, "type": instance.notification_type}
        if instance.order_id:
            payload["order_id"] = instance.order_id
        if instance.order_item_id:
            payload["order_item_id"] = instance.order_item_id
        send_push_to_user(
            user=instance.user,
            title=instance.title or "Trenva",
            body=instance.message or "You have a new update.",
            data=payload,
        )
    except Exception:
        # Push failure should never block core flow.
        pass
